...
from src.data.tackler_info import (
    simplify_tackles_df,
    join_ball_carrier_tracking,
    dist_calc,
    tackler_distance_frame,
    contact_behind,
)
...
def main(input_directory, output_filepath, play_type):
    ...
    weight_df = standard_tracking_df.merge(
        players_df[["nflId", "weight", "position"]], on="nflId"
    )
    ...
    ball_carrier_dist_df = dist_calc(
        defenders_tracking_df, name="tackler_to_ball_carrier_dist"
    )
    tackle_simple_df = simplify_tackles_df(tackles_df)
    tackler_dist_df = tackler_distance_frame(
        tackle_simple_df, ball_carrier_dist_df, dist=2
    )
...
def reduce_tracking_data(df, plays_df, play_type="run"):
    ...
    tackle_frame_df = df[df["event"] == "tackle"][
    ...
    tracking_to_tackle_df = temp_tracking_df[
        temp_tracking_df["tackle_frame"] > temp_tracking_df["frameId"]
    ]

    # reduce dataframe to selected play type
    if play_type == "run":
        run_plays_df = plays_df[plays_df["passResult"].isnull()]
        reduced_tracking_df = run_plays_df[["gameId", "playId"]].merge(
            tracking_to_tackle_df, on=["gameId", "playId"]
        )
    elif play_type == "pass":
        # before evaluating pass plays, reduce the tracking data to only frames after the catch
        pass_caught_df = tracking_to_tackle_df[
            tracking_to_tackle_df.event == "pass_outcome_caught"
        ][["gameId", "playId", "frameId"]].drop_duplicates(keep="first")
        pass_caught_df.rename(columns={"frameId": "catch_frame"}, inplace=True)
        temp_tracking_df_pass = tracking_to_tackle_df.merge(
            pass_caught_df, on=["gameId", "playId"]
        )
        pass_caught_to_tackle_df = temp_tracking_df_pass[
            temp_tracking_df_pass["catch_frame"] < temp_tracking_df_pass["frameId"]
        ]

        pass_plays_df = plays_df[plays_df["passResult"] == "C"]
        reduced_tracking_df = pass_plays_df[["gameId", "playId"]].merge(
            pass_caught_to_tackle_df, on=["gameId", "playId"]
        )
    else:
