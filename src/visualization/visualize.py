...
def animate_play(tracking_df, play_df, players, gameId, playId):
    ...
    tracking_players_df = pd.merge(
        tracking_df, players, how="left", on="nflId", suffixes=[None, "_y"]
    )
...
def animate_frame(
    ...
    ball_carrier_df = selected_tracking_df[
        (selected_tracking_df.frameId == frameId)
    ...
    defender_df = selected_tracking_df[
        (selected_tracking_df.frameId == frameId)
    ...
    shift_scale = 4
    x1 = (
        -shift_scale * np.cos(ball_carrier_dir)
        + shift_scale * np.sin(ball_carrier_dir)
        + ball_carrier_x
    )
    x2 = (
        -shift_scale * np.cos(ball_carrier_dir)
    ...
    for team in selected_tracking_df.club.unique():
        ...
        if team != "football":
            ...
            if animation_image >= arrow_image:
                ...
                plot_df["x_change"] = plot_df["x"] + (
                    plot_df["s"] * np.cos(dir_radians)
                )
                plot_df["y_change"] = plot_df["y"] + (
                    plot_df["s"] * np.sin(dir_radians)
                )
                ...
                plot_df["x_change_bc"] = plot_df["x"] + (
                    plot_df["s"] * np.cos(contact_angle_rad)
                )
                plot_df["y_change_bc"] = plot_df["y"] + (
                    plot_df["s"] * np.sin(contact_angle_rad)
                )

                plot_df["parallel_point"] = plot_df.apply(
                    lambda x: rotate_point(
                        x["x"], x["y"], x["x_change_bc"], x["y"], ball_carrier_dir
                    ),
                    axis=1,
                )
                plot_df["perpendicular_point"] = plot_df.apply(
                    lambda x: rotate_point(
                        x["x"], x["y"], x["x"], x["y_change_bc"], ball_carrier_dir
                    ),
                    axis=1,
                )
                ...
                plot_df[
                    ["x_perpendicular_point", "y_perpendicular_point"]
                ] = pd.DataFrame(
                ...
                x_perpendicular = plot_df[
                    ["x", "x_perpendicular_point"]
                ].values.tolist()
                y_perpendicular = plot_df[
                    ["y", "y_perpendicular_point"]
                ].values.tolist()
    ...
    layout = go.Layout(
        ...
        yaxis=dict(
            range=layout_y_range, autorange=False, showgrid=False, showticklabels=False
        ),
        plot_bgcolor="#00B140",
        # Create title and add play description at the bottom of the chart for better visual appeal
    ...
    for y_val in first_down_markers:
        fig.add_annotation(
            ...
            text=str(down),
            showarrow=False,
            font=dict(family="Arial Black", size=14, color="#ff7f0e"),
            align="center",
            bordercolor="black",
            borderwidth=0,
            borderpad=2,
    ...
    if animation_image >= plane_of_field_image:
        ...
        fig.add_annotation(
            ...
            showarrow=False,
            font=dict(family="Droid Sans", size=20, color="white"),
            align="center",
        )
