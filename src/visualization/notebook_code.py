...
#     metrics_df.drop(metrics_df[metrics_df['event'] == 'forcedFumble'].index, inplace=True)
    metrics_df.drop(metrics_df[metrics_df['event'].isin(['forcedFumble', 'None'])].index, inplace=True)
...
def regression(df, metric, fig_name=None):
    ax = sns.scatterplot(df, x=f"Defender {metric}", y=f"Ball Carrier {metric}", 
                         hue='Outcome', hue_order=df['Outcome'].unique(), alpha=0.3)
    for event_type in df['Outcome'].unique():
        plotting_df = df.copy()
        plotting_df = plotting_df[plotting_df['Outcome'] == event_type]
        sns.regplot(plotting_df, x=f"Defender {metric}", y=f"Ball Carrier {metric}", scatter=False)
    if fig_name:
        plt.savefig(fig_name)
    plt.show()
...
def helmet_title_imgs(adjust):
    ...
    ab_1 = AnnotationBbox(imagebox_helmet_1, (0.15, 0.87+adjust[1]),
                          xycoords='figure fraction',
                          frameon=False)

    imagebox_football_1 = OffsetImage(img_football, zoom=0.05-adjust[2])
    ab_2 = AnnotationBbox(imagebox_football_1, (0.13, 0.87+adjust[1]),
                          xycoords='figure fraction',
                          frameon=False)

    imagebox_helmet_2 = OffsetImage(ImageOps.mirror(img_helmet_red), zoom=0.15-adjust[0])
    ab_3 = AnnotationBbox(imagebox_helmet_2, (0.32, 0.87+adjust[1]),
                          xycoords='figure fraction',
                          frameon=False)

    imagebox_helmet_3 = OffsetImage(img_helmet, zoom=0.15-adjust[0])
    ab_4 = AnnotationBbox(imagebox_helmet_3, (0.62, 0.87+adjust[1]),
                          xycoords='figure fraction',
                          frameon=False)

    imagebox_football_2 = OffsetImage(img_football, zoom=0.05-adjust[2])
    ab_5 = AnnotationBbox(imagebox_football_2, (0.6, 0.87+adjust[1]),
                          xycoords='figure fraction',
                          frameon=False)

    imagebox_helmet_4 = OffsetImage(img_helmet_red, zoom=0.15-adjust[0])
    ab_6 = AnnotationBbox(imagebox_helmet_4, (0.78, 0.87+adjust[1]),
                          xycoords='figure fraction',
                          frameon=False)
...
def endzone_plot(sampled_df, fig_name=None):
    ...
    quad_plot_momentum(plot_1_df, ax[1], 'Defender Endzone Momentum (abs)', 'Ball Carrier Endzone Momentum (abs)')
    plot_2_df = sampled_df[(sampled_df['momentum_x'] < 0) & (sampled_df['momentum_x_ball_carrier'] > 0)]
    quad_plot_momentum(plot_2_df, ax[0], 'Defender Endzone Momentum (abs)', 'Ball Carrier Endzone Momentum (abs)')
...
def parallel_plot(sampled_df, fig_name=None):
    ...
    quad_plot_momentum(plot_1_df, ax[0, 1], 'Defender Parallel Momentum (abs)', 'Ball Carrier Momentum')
    plot_2_df = sampled_df[(sampled_df['contact_angle_momentum'] < 0)]
    quad_plot_momentum(plot_2_df, ax[0 ,0], 'Defender Parallel Momentum (abs)', 'Ball Carrier Momentum')
    plot_3_df = sampled_df[(sampled_df['momentum_x'] > 0) & (sampled_df['momentum_x_ball_carrier'] > 0)]
    quad_plot_momentum(plot_3_df, ax[1, 1], 'Defender Endzone Momentum (abs)', 'Ball Carrier Endzone Momentum (abs)')
    plot_4_df = sampled_df[(sampled_df['momentum_x'] < 0) & (sampled_df['momentum_x_ball_carrier'] > 0)]
    quad_plot_momentum(plot_4_df, ax[1, 0], 'Defender Endzone Momentum (abs)', 'Ball Carrier Endzone Momentum (abs)')
...
def parallel_plot_2(sampled_df, fig_name=None):
    ...
    plt.subplots_adjust(top=5/7, hspace=2/7, wspace=0.4)
    plot_1_df = sampled_df[sampled_df['contact_angle_momentum'] > 0]
    quad_plot_momentum(plot_1_df, ax[1], 'Defender Parallel Momentum (abs)', 'Ball Carrier Momentum')
    plot_2_df = sampled_df[(sampled_df['contact_angle_momentum'] < 0)]
    quad_plot_momentum(plot_2_df, ax[0], 'Defender Parallel Momentum (abs)', 'Ball Carrier Momentum')
...
def xy_plots(fig_name=None):
    ...
    plt.subplots_adjust(wspace=0.3)
    plot_1_df = sampled_df[sampled_df['contact_angle_momentum'] > 0]
    quad_plot_momentum(plot_1_df, ax[2,1], 'Defender Parallel Momentum (abs)', 'Defender Perpendicular Momentum (abs)', True)
    plot_2_df = sampled_df[(sampled_df['contact_angle_momentum'] < 0)]
    quad_plot_momentum(plot_2_df, ax[2,0], 'Defender Parallel Momentum (abs)', 'Defender Perpendicular Momentum (abs)')
    plot_3_df = sampled_df[sampled_df['momentum_x'] > 0]
    quad_plot_momentum(plot_3_df, ax[1,1], 'Defender Endzone Momentum (abs)', 'Defender Sideline Momentum (abs)')
    plot_4_df = sampled_df[(sampled_df['momentum_x'] < 0)]
    quad_plot_momentum(plot_4_df, ax[1,0], 'Defender Endzone Momentum (abs)', 'Defender Sideline Momentum (abs)')
    plot_5_df = sampled_df[sampled_df['momentum_x_ball_carrier'] > 0]
    quad_plot_momentum(plot_5_df, ax[0,1], 'Ball Carrier Endzone Momentum (abs)', 'Ball Carrier Sideline Momentum (abs)')
    plot_6_df = sampled_df[(sampled_df['momentum_x_ball_carrier'] < 0)]
    quad_plot_momentum(plot_6_df, ax[0,0], 'Ball Carrier Endzone Momentum (abs)', 'Ball Carrier Sideline Momentum (abs)')
    handles, labels = ax[0,1].get_legend_handles_labels()
    fig.legend(handles, labels, loc='upper right')
    ax[0,0].add_artist(ab_1)
    ax[0,0].add_artist(ab_2)
...
def plane_comparison(run_metrics_df, fig_name=None):
    ...
    fig, axes = plt.subplots(nrows=1, ncols=2, figsize=(11, 5), subplot_kw={'projection': 'polar'})
    plt.subplots_adjust(wspace=0.3)
    r = defender_df.momentum
    area = defender_df.momentum/2
    colors = defender_df.apply(lambda x: 'orange' if x.event == 'tackle' else 'blue', axis=1)
    ...
    # Add Legend
    colors = {'Tackle':(1.0, 0.6470588235294118, 0.0, 0.3), 'Missed Tackle':(0.0, 0.0, 1.0, 0.3)}         
    ...
    ab_1 = AnnotationBbox(imagebox_helmet_1, (0.5, 0.5),
                          xycoords='axes fraction',
                          frameon=False)
    axes[0].add_artist(ab_1)

    imagebox_helmet_2 = OffsetImage(img_helmet_red, zoom=0.09)
    ab_2 = AnnotationBbox(imagebox_helmet_2, (0.5, 0.5),
                          xycoords='axes fraction',
                          frameon=False)
    ...
    ab_3 = AnnotationBbox(imagebox_helmet_3, (0 - 0.04, 1.1 - 0.06),
                          xycoords='axes fraction',
                          frameon=False)
    axes[1].add_artist(ab_3)

    imagebox_football_1 = OffsetImage(img_football, zoom=0.03)
    ab_4 = AnnotationBbox(imagebox_football_1, (-0.04 - 0.04, 1.1 - 0.06),
                          xycoords='axes fraction',
                          frameon=False)
    axes[1].add_artist(ab_4)

    imagebox_field_1 = OffsetImage(img_field, zoom=0.4 - 0.05)
    ab_5 = AnnotationBbox(imagebox_field_1, (-0.04 + 0.01, 1.1 - 0.05),
                          xycoords='axes fraction',
                          frameon=False)
    ...
    ab_6 = AnnotationBbox(imagebox_helmet_4, (1, 0.5),
                          xycoords='axes fraction',
                          frameon=False)
    axes[1].add_artist(ab_6)

    imagebox_football_2 = OffsetImage(img_football, zoom=0.015)
    ab_7 = AnnotationBbox(imagebox_football_2, (0.98, 0.5),
                          xycoords='axes fraction',
                          frameon=False)
    axes[1].add_artist(ab_7)
    
    fig.text(0.45, 0.4, "TOUCHDOWN", 
             fontsize=10, 
    ...
    fig.text(0.515, 0.15, "OLB Preston Smith", fontsize=14,
            horizontalalignment='center')
    if fig_name:
        plt.savefig(fig_name)
...
def some_code():
    ...
    player_tackles = run_metrics_df.groupby(['nflId', 'event'], as_index=False)['playId'].agg({'playId': 'count'})
    player_tackles.rename({'playId': 'count'}, axis=1, inplace=True)
    player_tackles['percent'] = player_tackles['count']/player_tackles.groupby('nflId')['count'].transform('sum')
    player_pivot = player_tackles.pivot(columns='event', index='nflId', values=['count', 'percent']).dropna()
